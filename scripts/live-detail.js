...
fetch('../../data/itemData.json')
    ...
    .then(data => {
        const flower = data.categories.find(category => category.category === 'Live Plants')
        ...
        if (flower) {
            ...
            document.getElementById('flower-image').innerHTML = `
                <img src="../../assets/img/our_items/live_plants/${flower.image}" class="sm:w-full sm:h-screen border object-cover" alt="${flower.title}">
            `;
