...
fetch('../../data/itemData.json')
    ...
    .then(data => {
        const flower = data.categories.find(category => category.category === 'Fresh Flowers')
        ...
        if (flower) {
            ...
            document.getElementById('flower-image').innerHTML = `
                <img src="../../assets/img/our_items/fresh_flower/${flower.image}" class="sm:w-full sm:h-screen border object-cover" alt="${flower.title}">
            `;
